...
(function($) {
	...
	var _initHeaders = function() {
		...
		var prevMLink = $('<div class="MonthNavPrev"><a href="" class="link-prev">'+ def.navLinks.p +'</a></div>').click(function(e) {
			e.preventDefault();
			e.stopPropagation();

			$.jMonthCalendar.ChangeMonth(prevMonth);
			return false;
		});
		
		//Create Next Month link for later
		var nextMonth = _workingDate.clone().addMonths(1);
		var nextMLink = $('<div class="MonthNavNext"><a href="" class="link-next">'+ def.navLinks.n +'</a></div>').click(function() {
			$.jMonthCalendar.ChangeMonth(nextMonth);
			return false;
		});
		...
		if(def.navLinks.enablePrevYear) {
			prevYLink = $('<div class="YearNavPrev"><a href="">'+ prevYear.getFullYear() +'</a></div>').click(function() {
				$.jMonthCalendar.ChangeMonth(prevYear);
				return false;
			});
		}
		...
		if(def.navLinks.enableNextYear) {
			nextYLink = $('<div class="YearNavNext"><a href="">'+ nextYear.getFullYear() +'</a></div>').click(function() {
				$.jMonthCalendar.ChangeMonth(nextYear);
				return false;
			});
		}
		...
		if(def.navLinks.enableToday) {
			...
			todayLink = $('<div class="TodayLink"><a href="" class="link-today">'+ def.navLinks.t +'</a></div>').click(function() {
				$.jMonthCalendar.ChangeMonth(new Date().clearTime());
				return false;
			});
		}
	...
	$.jMonthCalendar.DrawCalendar = function(dateIn){
		var now = new Date();
		now.clearTime();
		...
		for (var i = 0; i < _totalBoxes; i++) {
			...
			//DateBox Events
			var dateLink = $('<div class="DateLabel">' + currentDate.getDate() + '</div>');
	...
	var _drawEventsOnCalendar = function() {
		...
		if (cEvents && cEvents.length > 0) {
			...
			$.each(cEvents, function(){
				...
				var startI = new TimeSpan(tempStartDT - _dateRange.startDate).days;// + (ev.StartDateTime.getMonth()==3? 1 : 0);
				var endI = new TimeSpan(tempEndDT.setHours(16) - _dateRange.startDate).days;// + (ev.EndDateTime.getMonth()==3? 1 : 0);

				// var startI = parseInt((convertDateToUTC(tempStartDT) - convertDateToUTC(_dateRange.startDate))/1000/60/60/24);
				// var endI = parseInt((convertDateToUTC(tempEndDT) - convertDateToUTC(_dateRange.startDate))/1000/60/60/24);
				if(ev.EventID)
				...
				for (var i = istart; i <= iend; i++) {
					...
					if (toManyEvents) {
						if (!b.isTooManySet) {
							...
							var index = i;
							moreDiv.css({ 
								"top" : (pos.top + (b.getCellBox().height() - b.getLabelHeight()) - 16 - 1), 
								"left" : pos.left, 
								"width" : (b.getLabelWidth()+1),
								"position" : "absolute" });
						...
						var pos = b.getCellPosition();
						block.css({ 
							"top" : (pos.top + b.getLabelHeight() + b.vOffset - 1), 
							"left" : pos.left, 
							"width" : (b.getLabelWidth()+1), 
							"position" : "absolute" });
	...
	$.jMonthCalendar.ClearEventsOnCalendar = function() {
		_clearBoxEvents();
	...
	$.jMonthCalendar.ReplaceEventCollection = function(eventCollection) {
		if(eventCollection) {
			cEvents = [];
	...
	$.jMonthCalendar.ChangeMonth = function(dateIn) {
		var changed = def.onMonthChanging.call(this, dateIn);
		if(!changed){
	...
	$.jMonthCalendar.Initialize = function(options, events) {
		var today = new Date();
